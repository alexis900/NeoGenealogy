...
```
 ...
 /trees/:treeId/research → Research Workspace (Overview: Research Sessions (Active/Planned, Current Session) + Research Activity (Sessions/Tasks/Outcomes/Evidence/Followups) + Planning preview + Opportunities/Active Tasks/Recent Outcomes + evidence/sources metrics)
 ...
 /trees/:treeId/research/sessions → Research Sessions (filtros status/person/opportunity, orden ACTIVE>PLANNED>COMPLETED>ABANDONED, tabs Active & Planned / History)
 /trees/:treeId/research/sessions/history → Research Session History (solo COMPLETED/ABANDONED, orden completed_at DESC fallback updated_at, filtros status/person_id, paginación, stats batch)
 /trees/:treeId/research/sessions/:sessionId → Research Session Detail (Objective/Person/Opportunity/Tasks + Session Summary (Tasks/Outcomes/Evidence/Follow-ups derived) + Progress + Task progress + Research Activity + Activity timeline 20 DESC)
...
- **Research Queue**: `getResearchOpportunities` con filtros `Priority, Sort(score/priority/confidence), min_score` → `ResearchOpportunityCard`; distinción `Research Queue` (automático) vs `Research Tasks` (humano).
 ...
 - **Research Workspace**: `getResearchSummary` + `getTasks(limit 5)` + `getOutcomes(limit 5)` + `getPlan(limit 3)` + `getSessions(limit 20)` → bloque **Research Sessions** (`Active / Planned` counts + `Current Session` con `terminal/total` y `Continue` + `View Sessions` + `View History`) + bloque **Research Activity** (Fase 5.3: `Sessions Active/Planned/Completed/Abandoned`, `Tasks Open/In Progress/Resolved`, `Outcomes Confirmed/False leads/Inconclusive/New leads/No evidence`, `Evidence Total/Supporting/Contradicting`, `Follow-ups Open/Completed/Skipped` desde `research_activity`) + bloque **What should I research next?** (top 3 `Planning Score` con link `View Research Plan` + fallback) + 3 bloques Opportunities (high/medium/low) + Active Tasks (OPEN/IN_PROGRESS) + Recent Outcomes (con gaps) + métricas `Evidence/Sources` + `Evidence Assessment` + `Evidence Gaps` + `Research Follow-ups` + `Follow-up Actions`. Nav incluye `Planning` + `Sessions` + `Session History`.
 ...
 - **Research Sessions**: `getSessions({status,person_id,opportunity_id})` → filtros `Status/Person/Opportunity`, orden `ACTIVE>PLANNED>COMPLETED>ABANDONED, updated_at DESC`, cards `status badge + title + person/opportunity`, tabs `Active & Planned / History`; **Session History** (Fase 5.3: `getSessionHistory({status,person_id,limit,page})` → solo `COMPLETED/ABANDONED`, orden `COALESCE(completed_at,updated_at) DESC`, stats batch, filtros, paginación, empty `No completed…` / `No sessions match…`); **Session Detail** `getSession` → `session + person + opportunity + tasks + summary (total/open/in_progress/terminal/outcomes) + stats (derived) + timeline (20 DESC)` con `Session Summary` (Tasks/Outcomes/Evidence/Follow-ups), `Research Activity`, `Task progress` + `Progress terminal/total`, `Activity` timeline + `Complete/Abandon/Reopen` (con confirmación y warning open tasks), `Remove Task`, `Delete`.
