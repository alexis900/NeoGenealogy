...
```
 ...
 /trees/:treeId/research/planning → Research Planning (What should I research next? Recommended/Deferred + Why is this here? + filtros priority/researchability/min_score + Start Research)
...
- **Research Queue**: `getResearchOpportunities` con filtros `Priority, Sort(score/priority/confidence), min_score` → `ResearchOpportunityCard`; distinción `Research Queue` (automático) vs `Research Tasks` (humano).
 ...
 - **Research Workspace**: `getResearchSummary` + `getTasks(limit 5)` + `getOutcomes(limit 5)` + `getPlan(limit 3)` → bloque **What should I research next?** (top 3 `Planning Score` con link `View Research Plan` + fallback explicativo si Planning no disponible) + 3 bloques Opportunities (high/medium/low) + Active Tasks (OPEN/IN_PROGRESS) + Recent Outcomes (con gaps) + métricas `Evidence/Sources` + `Evidence Assessment` + `Evidence Gaps` + `Research Follow-ups` + `Follow-up Actions` (Open/Completed/Skipped desde mismo summary, sin queries extra). Nav incluye `Planning`.
 - **Research Planning** (Fase 5.1): `getPlan({limit,min_score,priority,researchability})` → header **Research Planning — What should I research next?** + explicación `Planning combines existing Research Score, researchability, confidence, evidence gaps and current task state to suggest what is most useful to investigate next.` → bloque **Summary** compacto 6 métricas `Recommended / Candidates / Active research / Inconclusive / High priority / Critical gaps` (usa `ResearchPlanSummary` sin queries extra) → **Recommended** grid 2 col desktop / 1 col mobile, cards con jerarquía `Research Score (primario, ScoreBadge) > Priority > Planning Score (secundario, mono)` + `Researchability (HIGH/MEDIUM/LOW)` reutilizando badge + `Confidence 91%` (0.0..1.0 → %) + `Already being researched` / `Previously investigated · Inconclusive` + **Why is this here?** colapsable (`ResearchPlanningReason` del backend, no recálculo, `✓ label – description`) + acciones máx 2 `View Opportunity / Start Research` (→ `POST /research-opportunities/:id/tasks`, `Starting research…`, disable, `Research task created.` + refresh, maneja duplicado) o `View Research Task` si `active_task`/`INCONCLUSIVE` + **Deferred** colapsado por defecto (`Show deferred candidates`, lista compacta `Priority/Person/Title/Research Score/Planning Score` + `View Opportunity`) + filtros `Priority / Researchability / Min Planning Score (range+number 0..100) / Limit (10/20/50)` server-side + **URL state** `?priority=&researchability=&min_score=&limit=` (compartible, back/forward) + estados `Loading` skeleton, `No research opportunities to plan.` / `No opportunities match these filters. Try broadening` diferenciados, `Unable to load research planning. [Retry]` + responsive Tailwind + a11y (labels, aria-expanded, focus ring, no solo color).
...
`ResearchOpportunityCard, TaskStatusBadge, ScoreBadge, PriorityBadge, ConfidenceIndicator, ResearchabilityBadge, ScoreBreakdown, FindingBadge, SourceCoverageBar, BranchCard, PersonSummary, Pagination, Loading, ErrorState, Empty, ResearchPlanSummary, ResearchPlanCard, ResearchPlanReasons, DeferredResearchList, PlanningSkeleton`
...
- `web/src/pages/__tests__/ResearchTaskDetail.test.tsx` (detail, outcome: create/edit/delete, evidence SUPPORTS/CONTRADICTS, assessment statuses, gaps CRITICAL/WARNING/INFO, quick actions)
 - `web/src/pages/__tests__/ResearchPlanning.test.tsx` (planning: loading, empty/filtered-empty, header, summary 6 métricas, Recommended/Deferred, cards person/title/priority/Research Score/Planning Score/researchability/confidence, Why is this here? expand/collapse, active_task Already being researched + View Research Task, INCONCLUSIVE Previously investigated, Start Research POST/loading/success/duplicate, filters priority/researchability/min_score/limit + URL state, deferred collapsed/expand/navigation, error+retry, responsive)
