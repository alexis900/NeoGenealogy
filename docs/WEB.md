...
- **Research History**: `getOutcomes(limit 20, assessment_status, gap)` → tabla Date/Type/Summary/Task + `Evidence: N` + `Assessment: STATUS · score` + `Gaps: 1 warning` + `Follow-ups: 2` + `Follow-up actions: 2` (vía `followup_actions_count` sin N+1) + filtros, paginación.
