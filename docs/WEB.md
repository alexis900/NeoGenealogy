...
- **ResearchTask Detail**: `getTask`/`updateTask`/`deleteTask` + workflow `Start/Mark Resolved/Rejected/Inconclusive`; sección `Research Outcome` + `Evidence Assessment` (status/score/counts, `Why this assessment?`, warnings) + `Evidence Gaps` (CRITICAL/WARNING/INFO, títulos/descripciones, quick actions `Add Evidence`/`Review Evidence`/`Review Contradictions`, empty `No evidence gaps detected.`) + `Research Follow-ups` (HIGH/MEDIUM/LOW, `Add Evidence`/`Review Evidence`, oculto si `[]`, muestra `Gap: CODE` para trazabilidad) + `Evidence` (SUPPORTS/CONTRADICTS); muestra `Original Research Opportunity`.
- **Research History**: `getOutcomes(limit 20, assessment_status, gap)` → tabla Date/Type/Summary/Task + `Evidence: N` + `Assessment: STATUS · score` + `Gaps: 1 warning` + `Follow-ups: 2` + filtros type/person/assessment_status/gap (server-side gap), paginación, sin N+1.
