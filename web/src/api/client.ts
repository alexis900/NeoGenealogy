...
export const api = {
  ...
  // Research Outcomes
  getOutcomes: (treeId:number, params?:{type?:string;task_id?:number;person_id?:number;assessment_status?:string;gap?:string;limit?:number;offset?:number})=>{
    const q=new URLSearchParams();
    if(params?.type) q.set("type",params.type);
